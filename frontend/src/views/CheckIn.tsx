...
interface Props {
  ...
  onNote: (habitId: number, dateKey: string, note: string) => void;
}
...
export default function CheckIn({ habits, completions, onToggle, onNote }: Props) {
  ...
  const done = activeHabits.filter(h => (completions[todayKey] ?? []).includes(h.id)).length;
  ...
  const handleToggle = (habitId: number, e: React.MouseEvent<HTMLDivElement>) => {
    const rect = e.currentTarget.getBoundingClientRect();
    setRipple({ id: habitId, x: e.clientX - rect.left, y: e.clientY - rect.top });
    ...
    <div style={{ padding: '0 0 32px' }}>
      ...
      {activeHabits.length === 0 && (
        <div style={{ padding: '32px 20px', textAlign: 'center', color: 'var(--text-muted)', fontSize: 14 }}>
          No active habits. Add some in <strong style={{ color: 'var(--text-secondary)' }}>Habits</strong>.
        </div>
      )}

      <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {activeHabits.map(habit => {
          const checked = (completions[todayKey] ?? []).includes(habit.id);
